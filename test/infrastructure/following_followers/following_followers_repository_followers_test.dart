...
Stream<Either<FollowingFollowerErrorFailure, KtList<FollowingFollowers>>> mockFollowerUsers(MockFirestoreInstance instance, String userId) async* {
  yield* instance.collection('users').doc(userId).collection('followers')
      .snapshots()
      .map(
...
void main() {
  ...
  group('Test following followers repository for followers', () {
    test('Get followers successfully', () async {
      await instance.collection('users').doc(userId.getOrCrash()).collection('followers').add({
        'id' : userId.getOrCrash(),
        'username' : 'testname',
        'profileImageURL': 'testurl.com.jpeg',
      });
      Stream<Either<FollowingFollowerErrorFailure, KtList<FollowingFollowers>>> results = await mockFollowerUsers(instance, userId.getOrCrash());
      results.listen(
        expectAsync1(
              (event) {
            event.fold(
